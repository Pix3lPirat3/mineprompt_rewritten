...
`)
    ...
    let system_commands = commander.commands_array.filter(cmd => !cmd.requiresEntity).map(cmd => ({ command: cmd.command, usage: cmd.usage, description: cmd.description || 'This command does not have a description..' })).sort((a, b) => a.command.localeCompare(b.command))
    ...
    let player_commands = commander.commands_array.filter(cmd => cmd.requiresEntity).map(cmd => ({ command: cmd.command, usage: cmd.usage, description: cmd.description || 'This command does not have a description..' })).sort((a, b) => a.command.localeCompare(b.command))

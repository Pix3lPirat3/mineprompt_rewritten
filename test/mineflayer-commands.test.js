...
test('navigates to players and coordinates', async () => {
  ...
  await goto.execute(playerTarget.value, 'goto', ['builder', '4']);
  ...
  await goto.execute(coordinateTarget.value, 'goto', ['1', '70', '-8', '1']);
  ...
  const invalid = sender();
  await goto.execute(invalid.value, 'goto', ['x', '70', '-8']);
  assert.equal(invalid.replies[0], '[Goto] X must be a number.');
});

test('uses the client lifecycle for disconnects', async () => {
  let disconnected = false;
  global.mineflayer = { disconnect: async () => { disconnected = true; } };
  const response = sender();
  await disconnect.execute(response.value);
